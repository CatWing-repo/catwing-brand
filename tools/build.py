...
FONTS = {
    "body": {
        "stack": "'Poppins', -apple-system, BlinkMacSystemFont, sans-serif",
        "weights": [400, 500, 600, 700],
        "role": "Default UI, body text, headings",
    },
    "display": {
        "stack": "'Outfit', 'Poppins', sans-serif",
        "weights": [700],
        "role": "Logo wordmark, display headings (hero, login title)",
    },
    "mono": {
        "stack": "SFMono-Regular, Consolas, 'Liberation Mono', Menlo, monospace",
        "weights": [400, 500],
        "role": "Code blocks, hex values, data labels",
    },
}
...
# ── Tokens Export (for cross-repo agent consumption) ─────────────────────
...
def _export_tokens() -> tuple[Path, Path]:
    """Emit machine-readable token files alongside the HTMLs.

    Agents in sibling repos (catwing/, catwing-app/) consume tokens.json or
    tokens.css instead of re-deriving values from build.py / brand.html.
    Both files are deployed to GitHub Pages by .github/workflows/pages.yml.
    """
    payload = {
        "version": VERSION,
        ...
        "colors": {
            _slugify(name): {"hex": info["hex"], "role": info["role"], "group": info["group"]}
            for name, info in COLORS.items()
        },
        "tints": {_slugify(name): triple for name, triple in TINTS.items()},
        "gradients": {_slugify(name): info for name, info in GRADIENTS.items()},
        "context_palettes": {"admin": ADMIN_COLORS, "website": WEBSITE_COLORS},
        "type_scale": [
            {"name": name, "size": size, "weight": weight, "role": role}
            for (name, size, weight, role) in TYPE_SCALE
        ],
        "fonts": FONTS,
        "logos": {
            key: {"path": f"logo/{filename}", "filename": filename}
            for key, filename in LOGO_DIR_KEYS.items()
        },
    }

    json_path = BRAND_DIR / "tokens.json"
    json_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    css_lines = [
        "/* CatWing brand tokens — generated by tools/build.py. Do not edit. */",
        f"/* version {VERSION} · generated {payload['generated_at']} */",
        ":root {",
    ]
    for name, info in COLORS.items():
        css_lines.append(f"  --cw-color-{_slugify(name)}: {info['hex']};")
    for name, triple in TINTS.items():
        slug = _slugify(name)
        for part, value in triple.items():
            css_lines.append(f"  --cw-tint-{slug}-{part}: {value};")
    for name, info in GRADIENTS.items():
        css_lines.append(f"  --cw-gradient-{_slugify(name)}: {info['css']};")
    for role, info in FONTS.items():
        css_lines.append(f"  --cw-font-{role}: {info['stack']};")
    css_lines.append("}\n")
...
def main():
    ...
    json_path, css_path = _export_tokens()
    print(f"  Tokens: {json_path.name} ({json_path.stat().st_size / 1024:.1f} KB)")
    print(f"  Tokens: {css_path.name} ({css_path.stat().st_size / 1024:.1f} KB)")
